...
import {
  ...
  TECHSTARS_STAGE_SCHEMA_VERSION,
  writeStageDocument,
  type DiscoveredTechstarsCompany,
  type DiscoveredTechstarsDocument,
  type FilteredTechstarsCompany,
  type FilteredTechstarsDocument,
  type TechstarsLocationClassification,
} from "@/lib/techstars/staged-workflow";
import { writeTechstarsCompanyWorkbook } from "@/lib/techstars/stage-workbook";
import type {
  ExporterRegion,
  ExporterYear,
  SourceCandidate,
...
} from "@/lib/techstars/source-discovery";
...
} from "@/lib/techstars/source-cache-serialization";
import { mapWithConcurrency, SameHostRateLimiter } from "@/lib/500-global/rate-limiter";
import {
  validateTechstarsWorkbook,
  writeTechstarsWorkbook,
...
const CACHE_DIRECTORY = path.resolve(process.cwd(), ".cache", "techstars");
const MAX_SOURCE_CANDIDATES = 100;
const USER_AGENT =
  "FounderEmailVerifier/1.0 standalone-techstars-lead-exporter";
...
async function main() {
  ...
  if (!options) return;
  console.log(
    `Techstars exporter: stage=${options.stage} years=${options.years.join(",")} regions=${options.regions.join(",")} ${options.resume ? "resume" : options.fresh ? "fresh" : "standard"}`,
  );
...
async function runDiscoverStage(options: CliOptions) {
  ...
  console.log("\nStage 1/3 — discovering official Techstars companies...");
  ...
  const accepted = merged.companies.filter(
    (company) => Boolean(company.website && company.normalizedDomain),
  );
  ...
  for (const company of merged.companies) {
    if (!company.website || !company.normalizedDomain) {
      duplicateAudit.push({
        ...
        exclusion_reason: "Official participant entry has no usable company website/domain.",
  ...
  const document: DiscoveredTechstarsDocument = {
    schema_version: TECHSTARS_STAGE_SCHEMA_VERSION,
    stage: "discover",
    ...
    source_summary: {
      ...
      roster_locations_recognized: accepted.filter((company) =>
        Boolean(
          resolveTechstarsRosterLocation(company.listedCountry) ??
          extractTechstarsRosterLocation(company.description),
        ),
      ).length,
    },
...
async function runFilterStage(options: CliOptions) {
  ...
  const document: FilteredTechstarsDocument = {
    schema_version: TECHSTARS_STAGE_SCHEMA_VERSION,
    stage: "filter",
    generated_at: new Date().toISOString(),
    years: [...options.years],
  ...
  const workbookPath = path.resolve(process.cwd(), "exports", "Techstars_2025_2026_Companies.xlsx");
  await writeStageDocument(jsonPath, document);
  const counts = await writeTechstarsCompanyWorkbook(document, workbookPath);
...
async function runEnrichStage(options: CliOptions) {
  ...
  const compatibility = await writeTechstarsWorkbook({
    outputPath,
  ...
  await validateTechstarsWorkbook(outputPath, classified.ready);
...
function openStageCache(stage: "discover" | "filter" | "enrich", options: CliOptions, inputs: object) {
  return openExporterCache({
    ...
    optionsFingerprint: createOptionsFingerprint({
      schemaVersion: TECHSTARS_STAGE_SCHEMA_VERSION,
      stage,
      years: options.years,
...
function toDiscoveredCompany(company: ExporterCompanyRecord): DiscoveredTechstarsCompany {
...
function discoveredToCompanyRecord(company: DiscoveredTechstarsCompany): ExporterCompanyRecord {
  const rosterLocation =
    company.roster_location_evidence ??
    (() => {
      const resolved =
        resolveTechstarsRosterLocation(company.listed_location) ??
        extractTechstarsRosterLocation(company.description);
      return resolved
        ? {
...
function filteredToCompanyRecord(company: FilteredTechstarsCompany): ExporterCompanyRecord {
  return discoveredToCompanyRecord(company);
}

function toFilteredCompany(
  company: DiscoveredTechstarsCompany,
  enrichment: CompanyEnrichmentResult,
  ...
  const resolvedRoster =
    ...
    (() => {
      ...
      return resolved
        ? {
            authority: "techstars_official" as const,
  ...
  const classification: TechstarsLocationClassification =
  ...
  return {
    ...
    location_evidence: [
      ...(resolvedRoster
        ? [
            `${resolvedRoster.listed_location} → ${resolvedRoster.canonical_country} (techstars_official, participant_listing, confidence ${resolvedRoster.confidence.toFixed(2)}; ${resolvedRoster.source_url})`,
          ]
        : []),
...
function preserveFilteredLocation(
  ...
  filtered: FilteredTechstarsCompany,
): ExporterFinalCompany {
  return {
    ...company,
...
function toFilteredExcludedRow(company: FilteredTechstarsCompany): ExcludedExportRow {
  return {
    record_type: "company",
...
function printSourceCoverage(extractions: readonly ValidatedSourceExtraction[]) {
  ...
  console.log("Coverage by Techstars program:");
...
async function processCompany(
  ...
  if (
    ...
    return {
      company,
      enrichment: locationOnlyEnrichment(company, listedCountry, includeFounders),
    ...
    const enrichment = await enrichStandaloneCompany(
      {
        ...
        officialRosterAuthority: "techstars_official",
        includeFounders,
...
export function outputFilename(options: CliOptions) {
  ...
  const regions = options.regions
    ...
    .join("_");
  return `Techstars_${years}_${regions}.xlsx`;
}
...
function printHelp() {
  console.log(`Usage: npm run lead:techstars -- [options]
...
if (require.main === module) {
  void main().catch((error) => {
    console.error(`Techstars exporter failed: ${sanitizeError(error)}`);
    process.exitCode = 1;
  });
}
