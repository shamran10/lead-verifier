...
test("builds year-specific cohort, class, company, accelerator, program, and regional searches", () => {
  ...
  assert.equal(queries.every((query) => query.includes("site:techstars.com")), true);
  assert.equal(queries.some((query) => /NYC.*London.*Boston/i.test(query)), true);
  assert.equal(new Set(queries).size, queries.length);
});

test("official index discovery keeps Techstars cohort links and rejects unrelated roles", () => {
  const result = extractOfficialIndexCandidates(`
    <a href="/newsroom/meet-the-techstars-nyc-spring-2026-class">Meet the Techstars NYC Spring 2026 class</a>
    ...
    <a href="/newsroom/meet-the-2024-class">Meet the Techstars 2024 class</a>
  `, "https://www.techstars.com/newsroom", [2025, 2026]);
...
test("discovery drops explicitly stale-year URLs before they consume the source cap", async () => {
  const result = await discoverOfficialSourceCandidates({
    years: [2025, 2026],
    indexUrls: ["https://www.techstars.com/newsroom"],
    fetchIndex: async () => ({
      finalUrl: "https://www.techstars.com/newsroom",
