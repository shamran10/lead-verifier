...
export function isExplicitParticipantAssertion(value: string) {
  ...
  const scopedRosterHeading =
    /^(?:meet|introducing|presenting)?\s*(?:the\s+)?(?:techstars\s+)?(?:[\w-]+\s+){0,8}(?:batch\s*\d+|(?:spring|summer|fall|winter)?\s*\d{4}\s+(?:class|cohort)|(?:class|cohort)\s+(?:of\s+)?\d{4})(?:\s+(?:companies|startups|ventures|participants))?\s*:?$/i.test(
      text,
    );
...
function extractDescriptiveFields(
  ...
  const leadingRosterLocation = [
    ...
    .map(extractTechstarsRosterLocation)
  ...
  let listedCountry =
    ...
    null;
  const recognizedListedLocation = resolveTechstarsRosterLocation(listedCountry);
