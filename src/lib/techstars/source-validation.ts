...
const OFFICIAL_HOST = "techstars.com";
...
export function validateParticipantSource(
  ...
  if (!officialHost && !approvedPartnerHost) {
    return rejected(
      ["unapproved_host"],
      ["The source host is neither an official Techstars host nor an explicitly approved partner host."],
    );
  }
  ...
  if (!officialHost && !/\bTechstars\b/i.test(`${pageTitle ?? ""} ${mainText}`)) {
    reasons.push("missing_techstars_attribution");
  }
  ...
  const rosterIntent =
    /\b(?:class|cohort|demo\s+day|portfolio\s+update)\b/i.test(intentHeading) ||
    /\b(?:meet|introduc\w*|welcom\w*|announc\w*|selected)\b.{0,120}\b(?:startups?|companies|participants?)\b/i.test(
      intentHeading,
    ) ||
    /\bTechstars\s+Update\b/i.test(intentHeading);
...
function determineSourceYear(
  ...
  const focusedText = [
    ...
    ...mainText
      .split(/[.!?\n]/)
      .filter((segment) =>
        /\b(?:Techstars|accelerator|batch|class|cohort|demo\s+day|participants?|selected|startups?)\b/i.test(
          segment,
        ),
      )
      .slice(0, 100),
