...
import type { ExporterSourceKind } from "@/lib/techstars/exporter-types";
import { normalizeFounderName } from "@/lib/founder-normalization";
import { normalizeDomain } from "@/lib/founder-normalization";
import type { ReadyForUploadRow } from "@/lib/techstars/xlsx-export";
...
export function mergeValidatedSourceExtractions(
  ...
  for (const extraction of extractions) {
    for (const participant of extraction.participants) {
      if (!isPlausibleTechstarsCompanyName(participant.companyName)) {
        duplicates.push({
...
function isPlausibleTechstarsCompanyName(value: string) {
  const name = value.replace(/\s+/g, " ").trim();
  if (!name || name.length > 120) return false;
  if (/^\d+(?:[+%]|\s+countries?\s+represented)?$/i.test(name)) return false;
  if (/^Techstars\s+(?:and|partners?|update|accelerator|program)\b/i.test(name)) return false;
...
export function classifyFinalCompanies(
  ...
  for (const company of companies) {
    ...
    if (activeFounders.some((founder) => !founder.linkedinUrl)) {
      reviewReasons.push("One or more confirmed active founders are missing a company-published LinkedIn URL.");
    }
...
function isUploadCompatibleSource(value: string) {
  try {
    ...
    return url.protocol === "https:" && (host === "techstars.com" || host.endsWith(".techstars.com"));
  } catch {
    return false;
  }
}
