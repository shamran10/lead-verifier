...
test("generated workbook is accepted by the upload parser and diagnostics are skipped", async () => {
  ...
  const result = await writeTechstarsWorkbook({
    outputPath,
    ready: [{
    ...
    blockedSources: [{
      source_url: "https://www.techstars.com/blocked",
  ...
  assert.deepEqual(result.skippedSheets.sort(), ["Blocked Sources", "Excluded", "Needs Founder Review"]);
  ...
  const parsed = await parseWorkbook(
    new File([bytes], "techstars.xlsx"),
    "techstars",
  );
  assert.equal(parsed.founders[0]?.acceleratorName, "Techstars");
...
test("all four founder slots and exact company cells survive XLSX serialization", async () => {
  ...
  const ready = [{
    ...
    founder_4: "Dee Four",
    linkedin_url_4: "",
    source_url: "https://techstars.com/content/four-founder-company",
  }];
  const result = await writeTechstarsWorkbook({
    outputPath,
    ready,
