...
export async function writeTechstarsWorkbook(
  ...
  await writeWorkbookAtomically(input.outputPath, (filePath) =>
    writeXlsxFile(sheets, {
      sheets: ["Ready for Upload", "Needs Founder Review", "Excluded", "Blocked Sources"],
      columns: [
  ...
  return validateTechstarsWorkbook(input.outputPath, input.ready);
}

export async function validateTechstarsWorkbook(
  outputPath: string,
  ...
  const parsed = await parseWorkbook(file, "techstars");
  if (
