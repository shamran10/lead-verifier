...
import { OFFICIAL_TECHSTARS_SOURCES } from "@/lib/techstars/source-catalog-data";
...
const SOURCE_TERMS = /\b(class|cohort|batch|accelerator|demo[ -]?day|startups?|companies|participants?|presenting|selected|graduating)\b/i;
...
const OFFICIAL_INDEX_URLS = [
  "https://www.techstars.com/newsroom",
  ...Array.from(
    { length: 10 },
    (_, index) => `https://www.techstars.com/newsroom?page=${index + 2}`,
  ),
  "https://www.techstars.com/accelerators",
  "https://www.techstars.com/demoday",
...
export function buildSourceSearchQueries(years: readonly ExporterYear[]) {
  return years.flatMap((year) => [
    `site:techstars.com "${year} cohort" Techstars`,
    `site:techstars.com "${year} class" Techstars`,
    `site:techstars.com "${year} companies" Techstars accelerator`,
    `site:techstars.com "${year} accelerator" (cohort OR class OR companies)`,
    `site:techstars.com/newsroom ${year} ("meet the" OR "announces" OR "welcoming") (cohort OR class OR startups)`,
    `site:techstars.com/newsroom ${year} "demo day" (companies OR startups OR cohort)`,
    `site:techstars.com ${year} "portfolio update" companies Techstars`,
    `site:techstars.com ${year} (NYC OR London OR Boston OR Berlin OR Paris) (cohort OR class) Techstars`,
    `site:techstars.com ${year} (Seattle OR Chicago OR Toronto OR Montreal OR Calgary) (cohort OR class) Techstars`,
    `site:techstars.com ${year} (California OR Colorado OR Atlanta OR Baltimore OR Columbus) (cohort OR class) Techstars`,
    `site:techstars.com ${year} (Europe OR "North America") (cohort OR class OR companies) Techstars`,
  ]);
}
...
function isOfficialHost(hostname: string) {
  return hostname === "techstars.com" || hostname.endsWith(".techstars.com");
}
