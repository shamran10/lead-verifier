...
const discoveredCompany = {
  ...
  accelerator_batch: "Techstars 2025",
  accelerator_year: 2025 as const,
  program_name: "Techstars 2025",
  ...
  roster_location_evidence: {
    ...
    confidence: 0.98,
    source_url: "https://techstars.com/content/example",
  },
  industry: "Software",
  description: null,
  source_url: "https://techstars.com/content/example",
...
test("Stage 1 and Stage 2 documents persist a resumable founder-free boundary", async (t) => {
  const root = await mkdtemp(path.join(os.tmpdir(), "fev-techstars-stages-"));
  ...
  const discovered: DiscoveredTechstarsDocument = {
  ...
  const filtered: FilteredTechstarsDocument = {
    schema_version: TECHSTARS_STAGE_SCHEMA_VERSION,
    stage: "filter",
...
test("Stage 2 workbook retains eligible companies and separates unresolved and outside geography", async (t) => {
  const root = await mkdtemp(path.join(os.tmpdir(), "fev-techstars-filter-xlsx-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  ...
  const counts = await writeTechstarsCompanyWorkbook(document, outputPath);
...
test("stage documents reject raw HTML", async (t) => {
  const root = await mkdtemp(path.join(os.tmpdir(), "fev-techstars-safe-json-"));
  t.after(() => rm(root, { recursive: true, force: true }));
