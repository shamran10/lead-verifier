...
const EXCLUDED_ROLE_CONTAINER_NAME =
  /(?:^|[\s_-])(?:advisors?|investors?|judges?|mentors?|service[\s_-]?providers?|speakers?|sponsors?)(?:$|[\s_-])/i;
...
export function isExplicitParticipantAssertion(value: string) {
  ...
  const hasAssertion =
    /\b(?:announc(?:e|es|ed|ing)|cohort|finalists?|graduates?|graduating|lineup|meet|participants?|participating|presenting|selected|selection|showcas(?:e|ing)|spotlights?|winners?|startups?\s+(?:are|include)|companies\s+(?:are|include)|welcomes?)\b/i.test(
      text,
    );
  const scopedRosterHeading =
    /^(?:meet|introducing|presenting)?\s*(?:the\s+)?(?:masschallenge\s+)?(?:[\w-]+\s+){0,8}(?:batch\s*\d+|(?:spring|summer|fall|winter)?\s*\d{4}\s+(?:class|cohort)|(?:class|cohort)\s+(?:of\s+)?\d{4})(?:\s+(?:companies|startups|ventures|participants))?\s*:?$/i.test(
      text,
    );
...
export function isSimpleParticipantHeading(value: string) {
  return /^(?:the\s+)?(?:cohort|companies|finalists|participants|startups|ventures|winners)\s*:?$/i.test(
    cleanText(value),
  );
}
...
export function isExcludedRoleContainer(
  ...
  if ($(element).closest("nav, footer, aside").length) return true;
  const roleAncestor = $(element)
    .parents()
    .addBack()
    .toArray()
    .some((node) =>
      EXCLUDED_ROLE_CONTAINER_NAME.test(
        `${$(node).attr("class") ?? ""} ${$(node).attr("id") ?? ""}`,
      ),
    );
  if (roleAncestor) return true;
...
export function findMarkerElements(document: ExtractorDocument) {
  ...
  return $("h1, h2, h3, h4, h5, h6, p, strong")
    .toArray()
    .filter((element) => {
      const value = cleanText($(element).text()) || directText($, element);
      const isExplicit = isExplicitParticipantAssertion(value);
      if (
        (isExplicit && $(element).closest("nav, footer, aside").length > 0) ||
        (!isExplicit && isExcludedRoleContainer($, element))
      ) {
        return false;
      }
      return (
        value.length > 0 &&
        value.length <= 400 &&
        (isExplicit || isSimpleParticipantHeading(value))
      );
    });
}
...
export function extractParticipantFromEntry(
  ...
  const participantStatus = inferParticipantStatus(
    `${$("h1").first().text()} ${entry.closest("section, article, div").find("h2, h3, h4").first().text()} ${text}`,
  );
...
function inferParticipantStatus(
  value: string,
): ExtractedExporterParticipant["participantStatus"] {
  if (/\bfinalists?\b/i.test(value)) return "finalist";
  if (/\bwinners?\b/i.test(value)) return "winner";
  if (/\b(?:selected|chosen|accepted|welcom(?:e|es|ed|ing))\b/i.test(value)) {
    return "selected";
  }
  return "participant";
}
...
function extractDescriptiveFields(
  ...
  const leadingRosterLocation = [
    dataDescription,
    ...entry.find("p").toArray().map((element) => cleanText($(element).text())),
    text.replace(companyName, "").trim(),
  ]
    .map(extractMassChallengeRosterLocation)
  ...
  let listedCountry =
    explicitCountry ||
    genericLocation ||
    cleanText(countryLabel ?? "") ||
    leadingRosterLocation?.listedLocation ||
    null;
  const recognizedListedLocation = resolveMassChallengeRosterLocation(listedCountry);
  let listedCountryEvidence: "company_specific" | "ambiguous" | null =
    explicitCountry ||
    leadingRosterLocation ||
    (genericLocation && recognizedListedLocation) ||
    (/\b(?:headquarters|hq|location\s*:|based\s+in)\b/i.test(text) &&
      recognizedListedLocation)
      ? "company_specific"
      : listedCountry
        ? "ambiguous"
        : null;
