...
import {
  ...
  isCompletedFounderEnrichmentCacheValue,
  outputFilename,
  parseArguments,
  processSourceQueue,
  shouldUseLocationOnlyEnrichment,
} from "../../../scripts/find-masschallenge-leads";
import {
  createOptionsFingerprint,
  openExporterCache,
} from "@/lib/masschallenge/exporter-cache";
import { DiscoveryFetchError } from "@/lib/500-global/safe-fetch";
import { serializeSourceWorkResultForCache } from "@/lib/masschallenge/source-cache-serialization";
...
test("standalone CLI defaults and output filename match the upload workflow", () => {
  ...
  assert.equal(MASSCHALLENGE_STAGE_TWO_MAX_WEBSITE_CHECKS, 1);
  assert.equal(MASSCHALLENGE_STAGE_TWO_COMPANY_TIMEOUT_MS, 20_000);
  assert.equal(
    outputFilename(options),
    "MassChallenge_2025_2026_Europe_North_America.xlsx",
  );
});

test("the confirmed-location shortcut is Stage 2 only", () => {
  assert.equal(
    shouldUseLocationOnlyEnrichment(false, "company_specific", true),
    true,
  );
  assert.equal(
    shouldUseLocationOnlyEnrichment(true, "company_specific", true),
    false,
  );
  assert.equal(
    shouldUseLocationOnlyEnrichment(false, "ambiguous", true),
    false,
  );
});

test("resume reuses real founder enrichment but not location-only cache values", () => {
  const company = {
    website: "https://example.test/",
    normalizedDomain: "example.test",
  };
  assert.equal(
    isCompletedFounderEnrichmentCacheValue(company, { pagesAttempted: [] }),
    false,
  );
  assert.equal(
    isCompletedFounderEnrichmentCacheValue(company, {
      pagesAttempted: ["https://example.test/"],
    }),
    true,
  );
  assert.equal(
    isCompletedFounderEnrichmentCacheValue(
      { website: null, normalizedDomain: null },
      { pagesAttempted: [] },
    ),
    true,
  );
...
test("one source cache failure does not stop later source processing", async (t) => {
  ...
  const candidates = [
    { url: "https://masschallenge.org/content/first-2026" },
    { url: "https://masschallenge.org/content/second-2026" },
  ];
  const results = await processSourceQueue(
    ...
    {},
    {
      fetchSource: async (url) => ({
        finalUrl: url,
        html: "<html><title>MassChallenge 2026</title></html>",
        status: 200,
...
test("robots-blocked sources are recorded once and skipped safely", async (t) => {
  ...
  let fetches = 0;
  const url = "https://www.masschallenge.org/blocked";
  const results = await processSourceQueue(
    [{ url }, { url }],
    parseArguments([])!,
    cache,
