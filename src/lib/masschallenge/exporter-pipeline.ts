...
import type { ExporterSourceKind } from "@/lib/masschallenge/exporter-types";
import { normalizeFounderName } from "@/lib/founder-normalization";
import { normalizeDomain } from "@/lib/founder-normalization";
import type { ReadyForUploadRow } from "@/lib/masschallenge/xlsx-export";
...
export function mergeValidatedSourceExtractions(
  ...
  for (const extraction of extractions) {
    for (const participant of extraction.participants) {
      if (!isPlausibleMassChallengeCompanyName(participant.companyName)) {
        duplicates.push({
          companyName: participant.companyName,
          website: participant.website,
          acceleratorYear: extraction.source.acceleratorYear,
          reason: "Non-company statistic, program label, or newsroom navigation entry excluded.",
...
function isPlausibleMassChallengeCompanyName(value: string) {
  const name = value.replace(/\s+/g, " ").trim();
  if (!name || name.length > 120) return false;
  const words = name.split(/\s+/).filter(Boolean);
  if (words.length > 10 || /:\s*$/.test(name)) return false;
  if (/^\d+(?:[+%]|\s+countries?\s+represented)?$/i.test(name)) return false;
  if (/^MassChallenge\s+(?:and|partners?|update|accelerator|program)\b/i.test(name)) return false;
  if (/^(?:Founder Catalyst|Startup Weekend)\b/i.test(name)) return false;
  if (/^(?:program\s+)?(?:partners?|sponsors?|speakers?|mentors?|advisors?|investors?|judges?)(?:\s|$)/i.test(name)) return false;
  if (
    /^(?:access expansion|ai models? for|ai-powered tools|author|both events|challenge partners?|connect with|cyber security solutions?|de-risked|deep connections|diagnostics designed|energy efficiency|explore potential|faqs?|flexible financing|fraud detection|grant-ready|key dates|last article|masschallenge['’]s executive|patient access|platforms transforming|precision radiopharmaceuticals|program benefits|published on|solutions that|technology that|the seven founders|vaccine platforms|verizon on|wearable therapies|wearable ultrasound|workflow automation)\b/i.test(
      name,
    )
  ) {
    return false;
  }
  if (/^(?:Europe|Global|Remote|Switzerland|United Kingdom|United States)$/i.test(name)) {
    return false;
  }
  if (/\bProgram Components\b/i.test(name)) return false;
  return true;
}
...
function participantToCompany(
  ...
  return {
    ...
    programAssociations: [
      {
        program: extraction.source.acceleratorBatch,
        year: extraction.source.acceleratorYear,
        sourceUrl: extraction.source.url,
...
function mergeCompany(left: ExporterCompanyRecord, right: ExporterCompanyRecord) {
  ...
  return {
    ...
    programAssociations: dedupeProgramAssociations([
      ...(left.programAssociations ?? []),
      ...(right.programAssociations ?? []),
    ]),
...
function dedupeProgramAssociations(
  values: NonNullable<ExporterCompanyRecord["programAssociations"]>,
) {
  const byKey = new Map<string, (typeof values)[number]>();
  for (const value of values) {
    const key = `${value.year}\u0000${value.program ?? ""}\u0000${value.sourceUrl}\u0000${value.participantStatus}`;
    byKey.set(key, value);
  }
  return [...byKey.values()];
}
...
function isUploadCompatibleSource(value: string) {
  try {
    ...
    return url.protocol === "https:" && (host === "masschallenge.org" || host.endsWith(".masschallenge.org"));
  } catch {
    return false;
  }
}
