...
test("location-only enrichment in Stage 2 does not retain founder data", async () => {
  const result = await enrichStandaloneCompany(
    {
      companyName: "Location Only",
      website: "https://example.test",
      includeFounders: false,
      sourceFounders: [{
        name: "Roster Founder",
        role: "Founder & CEO",
        linkedinUrl: "https://www.linkedin.com/in/roster-founder",
        sourceUrl: "https://masschallenge.org/content/cohort",
      }],
    },
    {
      fetchHtml: async (url) => ({
        finalUrl: url,
        html: fixture("company-team.html"),
      }),
    },
  );
  assert.equal(result.founders.length, 0);
  assert.equal(result.activeFounders.length, 0);
  assert.equal(JSON.stringify(result).includes("Roster Founder"), false);
});

test("supports a one-page Stage 2 website-check limit", async () => {
  const calls: string[] = [];
  const result = await enrichStandaloneCompany(
    {
      companyName: "Bounded Location Check",
      website: "https://example.test",
      includeFounders: false,
    },
    {
      ...
      fetchHtml: async (url) => {
        calls.push(url);
        return { finalUrl: url, html: fixture("company-home.html") };
      },
    },
  );
  assert.equal(calls.length, 1);
  assert.equal(result.pagesAttempted.length, 1);
});
...
test("a clearly company-specific official roster country confirms current location", async () => {
  const result = await enrichStandaloneCompany(
    {
      ...
      officialSourceUrl: "https://masschallenge.org/content/official-cohort",
  ...
  assert.equal(result.locationEvidence[0]?.authority, "masschallenge_official");
});
  ...
  const result = await enrichStandaloneCompany(
    {
      ...
      sourceFounders: [
        ...
        },
        {
          name: "Alex Example",
          role: "Former Founder",
          sourceUrl: "https://masschallenge.org/content/update",
        },
      ],
    },
    {
