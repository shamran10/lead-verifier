...
import {
  ...
  parseArguments,
  processSourceQueue,
} from "../../../scripts/find-500-global-leads";
import {
  createOptionsFingerprint,
  openExporterCache,
} from "@/lib/500-global/exporter-cache";
import { DiscoveryFetchError } from "@/lib/500-global/safe-fetch";
import { serializeSourceWorkResultForCache } from "@/lib/500-global/source-cache-serialization";
...
test("standalone CLI defaults and output filename match the upload workflow", () => {
  ...
  assert.equal(
    ...
    "500_Global_2025_2026_Europe_North_America.xlsx",
  );
});
...
test("standalone CLI accepts safe subsets and conservative limits", () => {
  ...
  assert.equal(
    outputFilename(options),
    "500_Global_2026_North_America.xlsx",
  );
});
...
test("one source cache failure does not stop later source processing", async (t) => {
  ...
  const candidates = [
    { url: "https://500.co/content/first-2026" },
    { url: "https://500.co/content/second-2026" },
  ];
  const results = await processSourceQueue(
    ...
    {},
    {
      fetchSource: async (url) => ({
        finalUrl: url,
        html: "<html><title>500 Global 2026</title></html>",
        status: 200,
...
test("robots-blocked sources are recorded once and skipped safely", async (t) => {
  ...
  let fetches = 0;
  const url = "https://events.500.co/blocked";
  const results = await processSourceQueue(
    [{ url }, { url }],
    parseArguments([])!,
    cache,
