...
} from "@/lib/500-global/extractors";
import {
  cleanText,
...
export function validateParticipantSource(
  ...
  if (!officialHost && !approvedPartnerHost) {
    return rejected(
      ["unapproved_host"],
      ["The source host is neither an official 500 Global host nor an explicitly approved partner host."],
    );
  }
  ...
  if (!officialHost && !/\b500\s+Global\b/i.test(`${pageTitle ?? ""} ${mainText}`)) {
    reasons.push("missing_500_global_attribution");
  }
...
function determineSourceYear(
  ...
  const publicationYear = publishedDate
    ? Number(new Date(publishedDate).getUTCFullYear())
    : null;
  if (
    publicationYear !== null &&
    Number.isFinite(publicationYear) &&
    !targetYears.includes(publicationYear as ExporterYear)
  ) {
    return { year: null, reason: "unsupported_year" as const, warnings };
  }
  ...
  const focusedText = [
    ...
    ...mainText
      .split(/[.!?\n]/)
      .filter((segment) =>
        /\b(?:500\s+Global|accelerator|batch|cohort|demo\s+day|participants?|selected|startups?)\b/i.test(
          segment,
        ),
      )
      .slice(0, 100),
